        ...
        if flags.im_eval:
            ...
            if self.curr_stpes >= curr_max or self.terminate_state.sum() == humanoid_env.num_envs:
                ...
                if (humanoid_env.start_idx + humanoid_env.num_envs >= humanoid_env._motion_lib._num_unique_motions):
                    ...
                    if humanoid_env.humanoid_type  in ['smplx', 'smpl']:
                        L_hand_joints = ['L_Wrist', 'L_Index1', 'L_Index2', 'L_Index3', 'L_Middle1', 'L_Middle2', 'L_Middle3', 'L_Pinky1', 'L_Pinky2', 'L_Pinky3', 'L_Ring1', 'L_Ring2', 'L_Ring3', 'L_Thumb1', 'L_Thumb2', 'L_Thumb3']
                        R_hand_joints = ['R_Wrist', 'R_Index1', 'R_Index2', 'R_Index3', 'R_Middle1', 'R_Middle2', 'R_Middle3', 'R_Pinky1', 'R_Pinky2', 'R_Pinky3', 'R_Ring1', 'R_Ring2', 'R_Ring3', 'R_Thumb1', 'R_Thumb2', 'R_Thumb3']
                        L_hand_idx = [humanoid_env._body_names_orig.index(i) for i in L_hand_joints]
                        R_hand_idx = [humanoid_env._body_names_orig.index(i) for i in R_hand_joints]
                        pred_pos_all_pick = [i[:, L_hand_idx] for i in pred_pos_all_succ]; gt_pos_all_pick = [i[:, L_hand_idx] for i in gt_pos_all_succ]; 
                        print("Left hand:", {m: np.mean(v) for m, v in compute_metrics_lite(pred_pos_all_pick, gt_pos_all_pick).items()})
                        pred_pos_all_pick = [i[:, R_hand_idx] for i in pred_pos_all_succ]; gt_pos_all_pick = [i[:, R_hand_idx] for i in gt_pos_all_succ]; 
                        print("Right hand:", {m: np.mean(v) for m, v in compute_metrics_lite(pred_pos_all_pick, gt_pos_all_pick).items()})
...
class OmnigraspAMPPlayerContinuous(IMAMPPlayerContinuous):
    def __init__(self, config):
        super().__init__(config)
        self.gt_rot, self.gt_rot_all = [], []
        self.pred_rot, self.pred_rot_all = [], []
        if flags.im_eval:
            self.env.task.grab_termination_disatnce = torch.tensor([self.env.task.cfg.env.test_termination_distances]).to(self.env.task.grab_termination_disatnce) # if not humanoid_env.strict_eval else 0.25 # ZL: use UHC's termination distance
        ...
        if flags.im_eval:
            ...
            if self.curr_stpes >= curr_max or self.terminate_state.sum() == humanoid_env.num_envs:
                ...
                if (humanoid_env.start_idx + humanoid_env.num_envs >= humanoid_env._motion_lib._num_unique_motions):
                    ...
                    metrics_all = compute_metrics_obj(pred_pos_all, gt_pos_all, pred_rot_all, gt_rot_all)
                    metrics_succ = compute_metrics_obj(pred_pos_all_succ, gt_pos_all_succ, pred_rot_all_succ, gt_rot_all_succ)

                    metrics_all_print = {m: np.mean(v) for m, v in metrics_all.items()}
                    metrics_print = {m: np.mean(v) for m, v in metrics_succ.items()}
                    per_obj_fail_rate = humanoid_env._motion_lib.compute_per_obj_fail_rate(failed_keys)
                    print("------------------------------------------")
                    print("------------------------------------------")
                    print(f"Success Rate: {self.success_rate:.10f}")
                    print("All: ", " \t".join([f"{k}: {v:.3f}" for k, v in metrics_all_print.items()]))
                    print("Succ: "," \t".join([f"{k}: {v:.3f}" for k, v in metrics_print.items()]))
                    print("Obj_succ: "," \t".join([f"{k}: {1 - v:.3f}" for k, v in per_obj_fail_rate.items()]))
                    # print(1 - self.terminate_state.sum() / self.terminate_state.shape[0])
                    print(self.config['network_path'])
                    ...
                    eval_dump = {
                        ...
                        "pred_pos_all_succ": pred_pos_all_succ, 
                        "gt_pos_all_succ": gt_pos_all_succ,
                        "pred_rot_all": pred_rot_all,
                        "gt_rot_all": gt_rot_all,
                        "pred_rot_all_succ": pred_rot_all_succ,
                        "gt_rot_all_succ": gt_rot_all_succ,
                        
                        "success_lift": humanoid_env.success_lift.cpu().numpy(), 
                        "terminate_hist": terminate_hist,
                    }
